...
export const dataprivacys = createTable("dataprivacys", t => ({
  ...
  status: t.text("status").notNull(),
  breachSeverity: t.text("breach_severity"),
  status: t.text("status").notNull().default("active"),
  authorId: t.text("author_id").references(() => users.id, { onDelete: "set null" }),
  createdAt: t.timestamp("created_at").notNull().defaultNow(),
  updatedAt: t.timestamp("updated_at").notNull().defaultNow(),
...
export const relations = defineRelations({ users, sessions, accounts, farmers, crops, livestocks, programs, dataprivacys }, r => ({
  users: {
...
export const schema = Object.assign({ users, sessions, accounts, verifications, farmers, crops, livestocks, programs, dataprivacys }, relations)
